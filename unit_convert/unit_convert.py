...
def to_mhz_per_km(band_ghz, r_m, precision=2):
    ...
    """
    # MODIFY IN HERE ONLY
    band_ghz_r=band_ghz.replace("GHz","").replace("gHz","").replace('"','')
    band_mhz=float(band_ghz_r)*1000
    for each in r_m:
        if 'M' in each or 'm' in each:
            each1=each.replace('M','').replace('m','').replace(" ",'').replace("'","")
            temp=float(each1)/1000
            band_mhz=band_mhz/temp
    if precision > 0:
        return str(round(band_mhz,precision))
